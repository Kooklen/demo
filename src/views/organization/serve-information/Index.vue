  ...
  <div class="operat-platform">
    <div class="common-bg op_form filter-group">
      <div class="op_form_line filter-group-item">
        <IviewInput v-model="form.companyName" :label="'企业名称:'"></IviewInput>
        <IviewSelect v-model="form.companyType" :optionList="optionList" :label="'企业类型:'"></IviewSelect>
    ...
    <div class="common-bg op_table">
      ...
      <div class="op_table_box">
        <IviewTable :data="tableData" :columns="tableCol" :page="pager" @on-page-change="changePageSize"></IviewTable>
      </div>
    </div>

    <!-- S 添加 -->
    <div v-if="addCenter.visible">
      <addCenter @close="e => addModalCloseCallBack('addCenter')" :handleItem="handleItem"></addCenter>
    </div>
    <!-- E 添加 -->
    ...
    <IviewModal
      title="修改用户状态"
      type="confirm"
      v-model="resourceAssign.visible"
      @on-ok="isEnableCompany"
      @on-cancel="resourceAssign.visible = false"
      :confirmText2="`是否${handleItem.stauts ? '终止' : '恢复'}该企业服务?`"
    >
    </IviewModal>
  </div>
</template>
...
export default {
      ...
      tableCol: [
        ...cols([
          ...
          [
            ...
            {
              render: (h, p) => {
                let text = this.companyTypeDict[p.row.companyType] ? this.companyTypeDict[p.row.companyType].label : "-"
                return h("div", {}, text)
              }
            }
          ],
          [
            ...
            "county",
            {
              render: (h, p) => {
                ...
                text = text + "-" + (countyArr.length > 1 ? countyArr[1] : countyArr[0])
                return h("div", {}, text)
              }
            }
          ],
          ...
          [
            ...
            {
              render: (h, p) => {
                return h("div", {}, getWant("cooperationType", p.row.cooperationType))
              }
            }
          ],
          [
          ...
          [
            ...
            "action",
            {
              ...
              render: (h, p) => {
                let { row } = p
                let arr = []
                if (this.$permission("company-put")) arr.push("编辑")
                if (this.$permission("company-stop"))
                  arr.push({
                    label: row.status == 0 ? "恢复服务" : "终止服务",
                    options: {
                      style: { color: row.status == 0 ? "#70b603" : "#B8321A" }
                    }
                  })
  ...
  methods: {
    ...
    query() {
      ...
      let { companyName, contactName, timeArr, companyLicensesType } = this.form
      Object.assign(this.queryParams, {
        startDate: timeArr[0] ? this.$u.getDate(timeArr[0], "yyyymmdd", "-") : "",
        endDate: timeArr[1] ? this.$u.getDate(timeArr[1], "yyyymmdd") : timeArr[1],
        pageCurrent: current,
        pageSize,
        companyName,
        contactName,
    ...
    getList() {
      this.$api.enterpriseServe.sfGetCompanyList().then(res => {
        let { records, total } = res.data.data
        this.tableData = records
        this.pager.total = total
        this.queryResetPage = false
    ...
    // 终止或开启服务
    isEnableCompany() {
      let status = this.handleItem.status == 0 ? 1 : 0
      this.$api.enterpriseServe.putCompany({ ...this.handleItem, status }).then(res => {
        this.$Message.success("操作成功")
        this.resourceAssign.visible = false
        this.getList()
      })
    },
    changePageSize(e) {
    ...
    handleClickEdit(type, row) {
      let action = new Map([
        ["编辑", { key: "addCenter" }],
        ["完善信息", { key: "complete" }],
        ["终止服务", { key: "resourceAssign" }],
        ["恢复服务", { key: "resourceAssign" }]
      ])
    ...
    async optionInit() {
      ...
      this.optionList = this.$u.array.arrToSelect(res.data.data, "configName", "id")
    }
  }
}
</script>
