...
export default {
  ...
  created() {
    ...
    this.$on('tree-node-drag-over', (event, treeNode) => {
      ...
      let allowDrop = this.allowDrop

      if (this.disabledCrossLevel) {
        allowDrop = () => {
          return dropNode.node.parent.id === draggingNode.node.parent.id
        }
      }
      ...
      const expandIcon = dropNode.$el.querySelector(
        '.el-tree-node__expand-icon'
      )

      const iconPosition =
        expandIcon.getBoundingClientRect && expandIcon.getBoundingClientRect()
    ...
    this.$on('tree-node-drag-end', (event) => {
      ...
      if (this.disabledCrossLevel && dropType === 'inner') {
        return
      }
